...
final class EquipmentGroupCell: UITableViewCell {

    private let stackView = UIStackView()
    private var disposeBag = DisposeBag()
    private var valueChanged: ParameterBlock<Equipment>?
    ...
    func configure(with model: EquipmentGroupCellModel) {
        ...
        group1.rx.tap.subscribe { [weak self] _ in
            if let group1 = model.group1 {
                self?.valueChanged?(group1)
            }
        }
        ...
        group2.rx.tap.subscribe { [weak self] _ in
            if let group2 = model.group2 {
                self?.valueChanged?(group2)
            }
        }
        .disposed(by: disposeBag)
        ...
        group3.rx.tap.subscribe { [weak self] _ in
            if let group3 = model.group3 {
                self?.valueChanged?(group3)
            }
        }
        .disposed(by: disposeBag)

        let group4 = MuscleGroupButton(group: model.group4, isSelected: model.group4Selected)
        group4.rx.tap.subscribe { [weak self] _ in
            if let group4 = model.group4 {
                self?.valueChanged?(group4)
            }
        }
        .disposed(by: disposeBag)
        ...
        stackView.addArrangedSubview(group4)
    }
